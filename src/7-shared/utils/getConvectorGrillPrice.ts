...
export function getConvectorGrillPrice ({ model, radiator, grill }: Props): number {
  ...
  if (
    grill.price_id === 'basiс_grille_price'
    && radiator?.basiс_grille_price
  ) return parseInt(radiator.basiс_grille_price)

  if (
    grill.price_id === 'anod_color_grille_price'
    && radiator?.anod_color_grille_price
  ) return parseInt(radiator.anod_color_grille_price)

  if (
    grill.price_id === 'premium_grille_price'
    && radiator?.premium_grille_price
  ) return parseInt(radiator.premium_grille_price)

  if (
    grill.price_id === 'wood_grill_price'
    && radiator?.wood_grill_price
  ) return parseInt(radiator.wood_grill_price)
