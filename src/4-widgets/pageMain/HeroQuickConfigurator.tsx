...
function fieldLabelClass(): string {
	return 'mb-0.5 block text-xs font-medium uppercase tracking-[0.04em] text-neutral-600'
}
...
function InlineSelect({
	...
	return (
		...
		>
			...
			<button
				type='button'
				class={`mt-0 flex h-9 w-full items-center justify-between gap-2 rounded-lg border bg-neutral-50 px-2 text-left text-sm text-neutral-950 outline-none transition hover:border-neutral-300 focus:border-red-500 focus:ring-2 focus:ring-red-100 md:h-10 ${
						...
						{options.map(option => {
							...
							return (
								<button
									...
									class={`block w-full rounded-lg px-2 py-1 text-left text-xs leading-4 transition hover:bg-red-50 ${
		...
		>
			...
			{showConfiguratorBody && (
				...
				<div class='relative mt-0'>
					<button
						type='button'
						class='flex h-9 w-full items-center justify-between rounded-lg border border-neutral-200 bg-neutral-50 px-2 text-left text-sm outline-none transition hover:border-neutral-300 focus:border-red-500 focus:ring-2 focus:ring-red-100 md:h-10'
						aria-haspopup='listbox'
						aria-expanded={open}
						...
						<div class={`rounded-xl border border-neutral-200 bg-neutral-50 ${isModal ? 'p-2.5' : 'p-2.5 md:rounded-2xl md:p-3'}`}>
							...
							<div class={`mt-2 flex items-end justify-between gap-2.5 ${isModal ? '' : 'md:mt-2 md:gap-3'}`}>
								...
								{!isFloorModel && matchedVariants.length > 1 && (
									<a
										href={details.model.href}
										onClick={() => onNavigate?.()}
										class='text-right text-xs font-medium leading-4 text-red-700 hover:underline'
									>
							...
							<div class={`mt-2 flex flex-wrap items-center gap-1.5 ${isModal ? '' : 'md:mt-2'}`}>
								{itemInCartQnty > 0 ? (
									...
									<button
										type='button'
										class='h-9 flex-1 rounded-lg bg-red-700 px-3 text-sm font-semibold text-white transition hover:bg-red-800 disabled:cursor-not-allowed disabled:bg-neutral-300'
										disabled={!totalPrice}
										onClick={handleAddToCart}
									>
										{addToCartLabel}
