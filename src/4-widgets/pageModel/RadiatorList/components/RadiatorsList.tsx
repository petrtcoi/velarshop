...
function RadiatorList(props: Props) {
  ...
  const [lastFilterUpdate, setLastFilterUpdate] = useState<
    "length" | "height" | "width"
  >("height")
  const [selectedHeight, setSelectedHeight] = useState<string>(
    filterByHeight ? heights[0] : ALL
  )
  const [selectedWidth, setSelectedWidth] = useState<string>(
    filterByWidth ? widths[0] : ALL
  )
  const [selectedLength, setSelectedLength] = useState<string>(
    filterByLength ? lengths[0] : ALL
  )
  const [filteredRadiators, setFilteredRadiators] = useState<RadiatorJson[]>(
    initialFilteredRadiators
  )
  ...
  useEffect(() => {
    ...
    if (escapeRadiator) {
      ...
      if (lastFilterUpdate !== "width" && selectedWidth !== ALL)
        setSelectedWidth(escapeRadiator.width)
      if (lastFilterUpdate !== "length" && selectedLength !== ALL)
        setSelectedLength(escapeRadiator.length)
    }
  }, [filteredRadiators])
