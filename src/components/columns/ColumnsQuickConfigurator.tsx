...
function fieldLabelClass(): string {
	return 'text-xs font-normal text-neutral-600'
}

function selectClass(): string {
	return 'h-9 w-full cursor-pointer rounded-[3px] border border-neutral-200 bg-white px-2.5 text-xs text-neutral-900 outline-none transition-colors hover:border-red-300 focus-visible:border-red-500 focus-visible:ring-2 focus-visible:ring-red-100'
}
...
export default function ColumnsQuickConfigurator({ models }: Props) {
	...
	const heightOptions = useMemo(() => {
		const uniqueHeights = [...new Set(models.map(item => item.height).filter(value => value > 0))].sort((a, b) => a - b)
	...
	const model = useMemo(() => {
		return models.find(item => String(item.height) === height && String(item.tubeCount) === tubes)
	}, [models, height, tubes])
	...
	const canEstimate = Boolean(model) && Number.isFinite(sectionCount) && sectionCount > 0

	const matchedRadiator = useMemo(() => {
		if (!model || !Number.isFinite(sectionCount)) return undefined
		return radiatorsJsonData.find(
			radiator => radiator.model_id === model.id && Number.parseInt(radiator.sections || '0', 10) === sectionCount,
		)
	}, [model, sectionCount])
	...
	const estimatedPrice = hasExactPrice ? Math.round(basePrice * colorMultiplier + connectionPrice) : 0
	...
	const modelLabel = model ? `Velar ${model.name}` : 'нестандартная конфигурация'
	const colorLabel = selectedColor?.shortName ?? 'RAL'
	const connectionLabel = selectedConnection ? getConnectionUiLabel(selectedConnection) : 'Подключение под проект'
	...
	return (
		<div class='rounded-xl border-[1px] border-neutral-900 bg-white p-4 shadow-md md:p-5'>
			<h2 class='text-sm font-semibold text-neutral-950'>Быстрый подбор</h2>
			<p class='mt-1 text-xs leading-5 text-neutral-700'>Выберите параметры и сразу добавьте конфигурацию в корзину.</p>
				...
				<div class='grid grid-cols-1 gap-3 min-[380px]:grid-cols-2'>
					<div class='min-w-0'>
						...
						<select
							...
							class={selectClass()}
							value={height}
							onChange={event => setHeight((event.currentTarget as HTMLSelectElement).value)}
						>
				...
				<div class='grid grid-cols-1 gap-3 min-[380px]:grid-cols-2'>
					<div class='min-w-0'>
						...
						<select
							...
							class={selectClass()}
							value={color}
							onChange={event => setColor((event.currentTarget as HTMLSelectElement).value)}
						>
					...
					<div class='min-w-0'>
						...
						<select
							...
							class={selectClass()}
							value={connection}
							onChange={event => setConnection((event.currentTarget as HTMLSelectElement).value)}
						>
							{columnConnections.map(option => (
								<option value={option.id}>{getConnectionUiLabel(option)}</option>
							))}
						</select>
					</div>
				</div>
			...
			<div class='mt-4 rounded-lg border border-neutral-200 bg-neutral-100 p-3'>
				<div class='text-xs text-neutral-600'>Выбрано</div>
				<div class='mt-1 text-sm font-semibold text-neutral-950'>
					{modelLabel} · {sectionCount} секций · {colorLabel} · {connectionLabel.toLowerCase()}
				</div>

				<div class='mt-3 text-lg font-semibold text-neutral-950'>
					{hasExactPrice && estimatedPrice > 0
						? `Цена: ${estimatedPrice.toLocaleString('ru-RU')} ₽`
						: 'Точную стоимость подтвердит менеджер'}
				</div>

				<dl class='mt-3 space-y-1 text-xs text-neutral-700'>
				...
				{itemInCartQnty > 0 ? (
					<div class='flex flex-wrap items-center gap-3'>
						<div class='flex items-center gap-2'>
							...
							<div class='min-w-8 text-center text-sm font-semibold text-neutral-950'>{itemInCartQnty}</div>
							<button
								type='button'
								class='inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-[3px] border border-neutral-300 bg-white text-sm font-medium text-neutral-900 transition-colors hover:border-red-300  hover:text-red-700'
					...
					<button
						...
						class='inline-flex h-10 font-medium w-full !cursor-pointer items-center justify-center rounded-[3px] border border-red-700  px-4 text-sm hover:bg-red-50 text-red-700 transition-colors hover:border-red-800 hover:text-red-800 disabled:cursor-not-allowed disabled:border-neutral-300 disabled:opacity-70'
						onClick={handleAddToCart}
					>
						Добавить в корзину
					</button>
